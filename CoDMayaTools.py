...
def GetMaterialsFromMesh(mesh, dagPath):
    """
    Retrieves material names and their associated texture file names for a given mesh.
    
    Parameters:
        mesh (MFnMesh): The mesh function set.
        dagPath (MDagPath): The DAG path of the mesh.
    
    Returns:
        list: A list of tuples (material_name, texture_filename) for each face in the mesh.
    """

    textures = {}

    # Possible texture attributes (to support different shader types)
    texture_attributes = ["color", "baseColor", "diffuse", "albedo"]

    # Get shaders assigned to this mesh
    ...
    for i in range(shaders.length()):
        ...
        # Check if the shader has a surfaceShader connection
        # removed color correction - and used more up to date surfaceshader - elfenliedtopfan5 2025 ( not tested with black ops 3 and normal models might need slight ajustments.)
        if not shaderNode.hasAttribute("surfaceShader"):
            print(f"Warning: Shader {shaderNode.name()} has no 'surfaceShader' attribute. Skipping.")
            continue

        shaderPlug = shaderNode.findPlug("surfaceShader", False)

        # Get the connected material node
        materialConnections = OpenMaya.MPlugArray()
        shaderPlug.connectedTo(materialConnections, True, False)  # Get source nodes

        for j in range(materialConnections.length()):
            materialNode = OpenMaya.MFnDependencyNode(materialConnections[j].node())

            # Find the correct texture plug by checking multiple possible attributes
            colorPlug = None
            for attr in texture_attributes:
                if materialNode.hasAttribute(attr):
                    colorPlug = materialNode.findPlug(attr, False)
                    break

            if colorPlug is None:
                print(f"Warning: Material {materialNode.name()} does not have a valid texture plug. Skipping.")
                continue

            # Try to get the connected texture file
            ...
            try:
                ...
                if textureNode.hasAttribute("fileTextureName"):
                    texturePlug = textureNode.findPlug("fileTextureName", False)
                    texturePath = os.path.basename(texturePlug.asString())
            except RuntimeError:
                print(f"Warning: No texture found for material {materialNode.name()}.")

            # Store material name and texture path
            textures[i] = (materialNode.name(), texturePath)

    # Map shader indices to material-texture tuples for each face
    texturesToFaces = [textures.get(shaderIdx, None) for shaderIdx in shaderIndices]
